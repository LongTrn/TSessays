...
namespace Test2
{
    public partial class FrmTrangChu : Form
    {
        ...
        void ChangeAccountType(int Type)
        {
            if (Type != 1)
            {
                tcHome.Controls.Remove(tpAdmin);
        ...
        void UpdateAccountInfo() 
        {
            ...
            string newPw = AccountDAO.Instance.EncodedPassword(txtbPwMoi.Text);
            
            string rePw = AccountDAO.Instance.EncodedPassword(txtbRePw.Text);

            if(!newPw.Equals(rePw))
            {
                MessageBox.Show("Vui lòng nhập lại mật khẩu đúng và mật khẩu mối. !!!");
        ...
        private void btnThanhToan_Click_2(object sender, EventArgs e)
        {
            ...
            if (ord != null)
            {
                ...
                int idCustomer = 0;
                if (DataProvider.Instance.ExecuteNonQuery("SELECT id FROM dbo.Customer WHERE nameCustomer = N'" + txtbTenKH.Text + "' ") <= 0)
                {
                    idCustomer = -1;
                }
